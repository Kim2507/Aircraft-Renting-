...
public:
    ...
    MidSize(std::string name, int num_pass, std::string iednurance, int ibag_capacity, double width, double length, double height, double speed, std::string commodations) :Fleet(name, num_pass, iednurance, ibag_capacity, width, length, height, speed) {
        setCommodations(commodations);
    }

    //Destructor ( always makes destructor virtual)
    virtual ~MidSize() = default;                    // Set default bc we do not do any work on destructor
